...
export const Config: Schema<Config> = Schema.intersect([
  Schema.object({
    ...
    updateInterval: Schema.number()
      ...
      .default(5)
      .description(
        "检查推文更新间隔时间（单位分钟），建议每多两个订阅增加1分钟"
      ),
    cookies: Schema.string()
      .required()
      .description("x的登录cookies，获取方式往上翻看简介"),
  }).description("基础设置"),

  Schema.object({
    whe_translate: Schema.boolean()
      .default(false)
      .description("是否启用推文翻译（接入deepseek v3）"),
  }).description("翻译设置"),
  ...
  Schema.union([
    Schema.object({
      ...
      apiKey: Schema.string()
        .required()
        .description(
          "deepseek apiKey密钥<br>点此链接了解👉https://platform.deepseek.com/api_keys"
        ),
      apiurl: Schema.string()
        .default("https://api.deepseek.com")
        .description(
          "默认为ds官方api接口，若使用siliconcloud平台请自行修改为https://api.siliconflow.cn/v1</br>"
        ),
      model: Schema.string()
        .default("deepseek-chat")
        .description(
          "默认为ds官方模型，若要切换为siliconflow平台对应模型，请上滑页面查看Notice"
        ),
    }),
    Schema.object({}),
  ]),
  ...
  Schema.object({
    bloggers: Schema.array(
      Schema.object({
        id: Schema.string().description(
          "Twitter博主用户名, 输@之后的用户名即可，不要加上@"
        ),
        groupID: Schema.array(String)
          .role("table")
          .description("需要推送的群号"),
      })
  ...
  Schema.object({
    outputLogs: Schema.boolean()
      .default(true)
      .description("日志调试模式，开启以获得更多信息")
      .experimental(),
  }).description("调试设置"),
]);
...
export async function apply(ctx: Context, config, session) {
  ...
  ctx.setInterval(async () => {
    checkTweets(session, config, ctx);
  }, config.updateInterval * 60 * 1000);

  ctx.command("tt", "主动检查一次推文更新").action(async ({ session }) => {
    await session.send("正在检查更新...");
    await checkTweets(session, config, ctx);
  });

  ctx.command("cs", "测试，开发专用").action(async ({ session }) => {
    await session.send("正在测试登录状态...");
    const result = await test(
      ctx,
      ctx.puppeteer,
      "https://x.com/xsjhsha/status/1957873302661390522",
      config
    );
  });

  ctx
    .command("twitter [...arg]", "根据url获得twitter推文截图")
    .action(async ({ session }, ...arg) => {
      try {
        ...
        if (url == "") {
          ...
          const shotcontent = await getScreenShot(
            ctx.puppeteer,
            url,
            config,
            ctx
          ...
          if (shotcontent.isVideo) {
              ...
              for (const videoUrl of shotcontent.mediaUrls) {
                ...
                while (attempts < maxRetries) {
                  try {
                    ...
                    attempts++;
                    logger.error(
                      `请求视频失败，正在尝试第 ${attempts} 次重试: ${videoUrl}`,
                      error
                    );
                  }
                }
            ...
            textMsg += `${h.image(shotcontent.screenshotBuffer, "image/webp")}`;
            if (shotcontent.mediaUrls && shotcontent.mediaUrls.length > 0) {
              const imagePromises = shotcontent.mediaUrls.map(
                async (imageUrl) => {
                  let attempts = 0;
                  const maxRetries = 3;
                  while (attempts < maxRetries) {
                    try {
                      ...
                      attempts++;
                      logger.error(
                        `请求图片失败，正在尝试第 ${attempts} 次重试: ${imageUrl}`,
                        error
                      );
                      if (attempts >= maxRetries) {
                        logger.error(
                          `请求图片失败，已达最大重试次数: ${imageUrl}`,
                          error
                        );
                        return null;
                      }
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          mediaUrls: apiResponse.media_extended
            ? apiResponse.media_extended.map((m) => m.url)
            : [],
          screenshotBuffer,
        };
      } catch (err) {
      ...
      const result = await page.evaluate(() => {
        ...
        for (const article of articles) {
          ...
          const isPinned = !!(
            ...
            Array.from(article.querySelectorAll("span")).some((s) =>
              /pinned|置顶|置頂/i.test(s.textContent || "")
            ) ||
            /pinned|置顶|置頂/i.test(
              ((article.previousElementSibling || {}).textContent || "") +
                ((article.parentElement || {}).textContent || "")
            )
          );
          ...
          const isVideo = !!(
            ...
            Array.from(
              ...
              /video|播放|影片|视频/i.test(n.getAttribute("aria-label") || "")
            )
          );
          let absolute = href;
          if (absolute.startsWith("/")) absolute = "https://x.com" + absolute;
          if (!absolute.startsWith("http"))
            absolute = "https://x.com/" + absolute;
...
async function checkTweets(session, config, ctx) {
  // 更新一次推文
  try {
    ...
    for (const blogger of config.bloggers) {
      ...
      try {
        ...
        // 判重
        const latestTweetLink =
          result.tweets.length > 0 ? result.tweets[0].link : null;
        const latestTweetcontent =
          result.tweets.length > 0 ? result.word_content : null;
        ...
        if (!existingTweetLink || existingTweetLink !== latestTweetLink) {
          ...
          // 获取具体内容
          const tpTweet = await getTimePushedTweet(
            ctx,
            ctx.puppeteer,
            latestTweetLink,
            config
          ...
          if (config.outputLogs) {
            ...
            logger.info(
              "推文媒体url:",
              tpTweet.mediaUrls.map((url) => url).join(", ")
            );
          }
          ...
          const isVideo =
            tpTweet.mediaUrls &&
            tpTweet.mediaUrls.some((url) => url.endsWith(".mp4"));
          // 根据config决定是否翻译推文
          let tweetWord;
                ...
                for (const videoUrl of tpTweet.mediaUrls) {
                  ...
                  while (attempts < maxRetries) {
                    try {
                      ...
                      await ctx.bots[botKey].sendMessage(
                        groupId,
                        h.video(response, "video/mp4")
                      );
        ...
        await session.send(
          `加载博主 ${id} 的页面时出错，可能是网络问题或链接不合法。请检查链接的合法性或稍后重试。`
        );
      }
    }
  } catch (error) {
...
async function init(config, ctx) {
  // 初始化数据库
  try {
    // 获取数据库中已存在的博主id，并过滤
    const existingIds = await ctx.database.get("xanalyse", {}, ["id"]);
    const existingIdSet = new Set(existingIds.map((item) => item.id));
    const newBloggers = config.bloggers.filter(
      (blogger) => !existingIdSet.has(blogger.id)
    );
    if (config.outputLogs) {
      logger.info(
        `[初始化]数据库中已存在的博主id：${Array.from(existingIdSet).join(
          ", "
        )}`
      );
      logger.info(
        `[初始化]需要初始化的博主id：${newBloggers
    ...
    for (const blogger of newBloggers) {
      ...
      try {
        const { tweets, word_content } = await getLatestTweets(
          ctx.puppeteer,
          bloggerUrl,
          config
        );
        ...
        logger.error(
          `加载博主 ${id} 的页面时出错，URL: ${bloggerUrl},请检查博主id是否正确，注意：id前不需要有@`,
          error
        );
      }
    }
...
async function getScreenShot(pptr, url, config, ctx) {
  // 获取指定x帖子截图
  let attempts = 0;
  let page;
  const maxRetries = 3;
...
async function translate(text: string, ctx, config) {
  // 翻译推文
  ...
  const data = {
    ...
    messages: [
      ...
      {
        role: "user",
        content: `翻译成简体中文，直接给出翻译结果，不要有多余输出不要修改标点符号，如果遇到网址或者空白内容请不要翻译，请翻译: ${text}`,
      },
    ],
    stream: false,
  };
...
async function test(ctx, pptr, url, config, maxRetries = 3) {
  // 测试用例：抓取 x.com 最新推文
  const tpTweet = await getTimePushedTweet(
    ctx,
    ctx.puppeteer,
